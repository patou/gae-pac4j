 ...
 */
package org.pac4j.gae.util;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpSession;

import org.pac4j.core.profile.CommonProfile;
import org.pac4j.gae.client.GaeUserServiceProfile;
...
public class UserUtils {
    ...
    /**
     * Return if the user is a global admin (Google app Engine Administrator).
     * 
     * @param session
     * @return if the user is a global Admin
     */
    public static boolean isGlobalAdmin(final HttpSession session) {
    	CommonProfile profile = getProfile(session);
		if (profile == null || (profile instanceof GaeUserServiceProfile))
			return false;
		return profile.getRoles().contains(GaeUserServiceProfile.PAC4J_GAE_GLOBAL_ADMIN_ROLE);
    }
